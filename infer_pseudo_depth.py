...
from pseudo_depth.LeReS import RelDepthModel, load_ckpt, SPVCNN_CLASSIFICATION, refine_focal, refine_shift
...
def parse_args():
    ...
    parser.add_argument(
        '--load_ckpt', default='weights/res101.pth', help='Checkpoint path to load')
    parser.add_argument('--input_dir', help='input images')
    parser.add_argument('--output_dir', help='output depths')
...
def reconstruct3D_from_depth(rgb, pred_depth, shift_model, focal_model):
    ...
    # recover focal
    focal_scale_1 = refine_focal(
        pred_depth_norm, proposed_scaled_focal, focal_model, u0=cam_u0, v0=cam_v0)
    predicted_focal_1 = proposed_scaled_focal / focal_scale_1.item()

    # recover shift
    shift_1 = refine_shift(pred_depth_norm, shift_model,
                           predicted_focal_1, cam_u0, cam_v0)
    shift_1 = shift_1 if shift_1.item() < 0.6 else torch.tensor([0.6])
    depth_scale_1 = pred_depth_norm - shift_1.item()

    # recover focal
    focal_scale_2 = refine_focal(
        depth_scale_1, predicted_focal_1, focal_model, u0=cam_u0, v0=cam_v0)
    predicted_focal_2 = predicted_focal_1 / focal_scale_2.item()

    return shift_1, predicted_focal_2, depth_scale_1


def inference(input_dir, output_dir, depth_model, shift_model, focal_model):
    ...
    rgb_files = sorted(
        sum([(input_dir).files('*.{}'.format(ext)) for ext in ['jpg', 'png']], []))

    for idx, f in enumerate(tqdm(rgb_files)):
        ...
        shift, focal, depth_scaleinv = reconstruct3D_from_depth(
            A_resize, pred_depth, shift_model, focal_model)
...
if __name__ == '__main__':
    ...
    inference(args.input_dir, args.output_dir,
              depth_model, shift_model, focal_model)
