# Generate leres depth as pseudo-depth for training SC-DepthV3
import argparse
import glob
import os
...
def inference_one_scene(scene_dir, depth_model, shift_model):
    ...
    for idx, f in enumerate(rgb_files):
        ...
        shift, depth_scaleinv = reconstruct3D_from_depth(
            rgb, pred_depth_ori, shift_model, focal_length)
...
if __name__ == '__main__':
    ...
    for name in sorted(os.listdir(args.dataset_dir)):
        if os.path.isdir(args.dataset_dir+name):
            print(name)
            inference_one_scene(args.dataset_dir+name,
                                depth_model, shift_model)
