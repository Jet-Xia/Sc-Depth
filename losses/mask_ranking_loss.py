...
class Mask_Ranking_Loss(nn.Module):
    ...
    def generate_percentMask_target(self, depth, pred, invalid_mask, theta=0.15):
        ...
        for bs in range(B):
            ...
            # 给每个 invalid 的点都选一个 valid 区域对应的点
        ...
        za_gt = gt_inval.clone()
        zb_pred = pred_val.clone()

        # 统一 pseudo-depth 与 pred-depth 的尺度
        ratio = torch.median(depth) / torch.median(pred)
        # 由于 pseudo-depth 背景还原的不好，depth值偏小，所以缩小一定倍率
        ratio = ratio / 1.15
        zb_pred *= ratio
    ...
    def get_unreliable(self, tgt_valid_weight):
        ...
        for bs in range(B):
            ...
            # 保护选中区域的坐标：让选中的坐标等于后面要淘汰的坐标
            indices[2040:2040 + int(unreliable_percent*H*W)] = indices[H*W-1]
            # use indices for the selection. mask=0 -> valid
            # 淘汰坐标：设为 0
            maskIv[indices] = 0
        # 返回是否大于 0 的 bool 矩阵，只有选中的区域没被设为0
    ...
    def forward(self, pred_depth, gt_depth, tgt_valid_weight):
        # dynamic mask
        unreliableMask = self.get_unreliable(tgt_valid_weight)   # [B, 1, H, W]
        ...
        total_loss = loss_percentMask / pointNum_1
        return total_loss
