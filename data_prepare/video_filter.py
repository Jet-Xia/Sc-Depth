# This script is used to filter out static-camera frames by checking the
# camera movement in video. We compute LK flow on sparse points those are
# detected in image (left and right boarders). The camera is regarded as
# static, if the average pixel shift is smaller than a threshold.
...
def compute_pixel_shift2(frame1, frame2):
    frame1_gray = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)
    frame2_gray = cv2.cvtColor(frame2, cv2.COLOR_BGR2GRAY)

    # Calculate Dense Optical Flow
    flow = cv2.calcOpticalFlowFarneback(
        frame1_gray, frame2_gray, None, 0.5, 3, 19, 3, 5, 1.2, 0
    )
